...
test_names = [
    "Network Weekly Report TEST.xlsx",
...
ref_names = [
    "tests/master_reference_file.xlsx",
...
class ListsSetupTest(unittest.TestCase):
    def setUp(self):
        base_settings = (
            " -q -pdf -s tests/test_settings.yaml -sv tests/test_settings_ssv.yaml"
        )
        ...
        for i in range(len(test_names)):
            if ref_names[i].endswith(".pdf"):
                ...
                for sheet in sheet_names:
                    self.assertFalse(
                        compare_excel_sheets(test_names[i], ref_names[i], sheet)
                    )
